// Fetches and parses real EasyList + EasyPrivacy (same lists AdGuard uses),
// caches in memory, and exposes matchers for both network + cosmetic blocking.
...
function parseList(raw: string) {
  ...
  raw.split('\n').forEach((line) => {
    ...
    // Network rule: ||domain.com^ (ignore option suffixes like $third-party)
    ...
    // Generic cosmetic hiding rule: ##.selector (no domain prefix = applies everywhere)
    if (line.startsWith('##')) {
  ...
  await Promise.all(
    LIST_URLS.map(async (url) => {
      try {
        ...
        // If a list fails to fetch, fall back silently to whatever we already have
      }
    })
  );

  domainSet = allDomains;
  // Cap selector count to keep the injected <style> block reasonable
